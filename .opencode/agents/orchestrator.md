...
- When `parallel_execution.enabled: true`, allow parallel specialist routing for independent work units, including `review_slices` and machine-readable conflict-plan `parallelGroup` categories, only if each unit has independent files, a merge/join plan, and grouped validation after join. Never parallelize delivery, schema/db migrations, shared contracts, unresolved public API decisions, or units that touch the same files.

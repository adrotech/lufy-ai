...
- For T1/T2 review boundaries with a known base, validate `context review --base <ref> --json` output when available: direct diff totals must remain observable, limits must identify `workflow_limits.review`, and stale/missing graph must yield unknown traceability plus recovery. Treat this as supporting evidence, never as proof that tests or gates passed.

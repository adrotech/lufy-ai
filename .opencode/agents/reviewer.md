...
- Use `project_profile.surfaces[*].architecture` to review architectural consistency: backend/fullstack code should match the selected `preferred` architecture, avoid mixing clean/hexagonal/controller-service-repository layers accidentally, and call out when implementation drifts from an already detected architecture.

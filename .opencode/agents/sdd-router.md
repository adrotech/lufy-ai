...
- **Security-sensitive routing guardrail**: requests that touch or likely touch runtime behavior or global configuration for CORS, authentication, authorization, JWT, sessions, cookies, CSRF, security headers, filters/middleware, roles, permissions/ACLs, allowed origins, ports, auth defaults, or global config are not direct T3. Set `fast_path_allowed: false` by default and classify at least T2; escalate to T1 when the change affects public contracts, cross-cutting security policy, auth defaults, architecture, or has unclear blast radius. T3 is allowed only for explicitly non-runtime/non-config documentation, tests, fixtures, comments, or mechanical updates.
...
skill_status:
  ...
  registry_status: ready | stale | not_available | unknown
  stack_detected:
    - <stack or unknown>

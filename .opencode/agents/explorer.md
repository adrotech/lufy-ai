...
- For multi-surface or ambiguous work, run or consume `lufy-ai plan --target <repo> --json` before manual surface interpretation. Carry its schema, source, primary/active surfaces, decisions, contracts and validation rule IDs in the handoff; the command is planning evidence, not validation evidence.

...
- **PR ignored/internal path gate** for `validator`/`delivery`: before push, PR creation, or PR-ready reporting, run `lufy-ai pr guard --base origin/develop` when the CLI is available; for pending local changes before commit, use `lufy-ai pr guard --base origin/develop --include-worktree`. This detects files in the PR diff that match `.gitignore` via `git check-ignore -v --no-index --stdin` and known internal metadata paths such as `openspec/`, `.lufy/`, `.lufy-ai/` and `pr_review/`. OpenSpec canonical specs under `openspec/specs/`, archive records under `openspec/changes/archive/`, and deleted active change files during archive are versionable for sync/archive PRs; active `openspec/changes/<id>/` files added or modified remain internal. The managed source root `.lufy/sdd/` is versionable because it feeds the installer catalog into `.lufy/workflows/sdd/`. Declared user-owned Lufy SDD roots under `.lufy/workflows/sdd/` (`changes/`, `specs/`, `decisions/`, `verification/`, `archive/`) are exempt only from the generic internal-prefix match; an explicit `.gitignore` match still blocks. If the CLI is unavailable, use the fallback `git diff --name-only origin/develop...HEAD -- | git check-ignore -v --no-index --stdin` plus the same manual classification. Block delivery or require explicit user override recorded in the PR/body when real matches appear, because `.gitignore` does not prevent already tracked files or cherry-picked commits from entering a PR.

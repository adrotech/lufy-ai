...
- **PR ignored/internal path gate** for `validator`/`delivery`: before push, PR creation, or PR-ready reporting, run `lufy-ai pr guard --base origin/develop` when the CLI is available; for pending local changes before commit, use `lufy-ai pr guard --base origin/develop --include-worktree`. This detects files in the PR diff that match `.gitignore` via `git check-ignore -v --no-index --stdin` and known internal metadata paths such as `openspec/`, `.lufy/`, `.lufy-ai/` and `pr_review/`. If the CLI is unavailable, use the fallback `git diff --name-only origin/develop...HEAD -- | git check-ignore -v --no-index --stdin` plus manual review of internal prefixes. Block delivery or require explicit user override recorded in the PR/body when matches appear, because `.gitignore` does not prevent already tracked files or cherry-picked commits from entering a PR.

...
Agents, workflow documentation, delivery policy and result contracts SHALL consume and report project workflow limits from `.lufy/config/project.yaml` top-level `workflow_limits` only.

#### Scenario: Router reads canonical workflow limits
- **WHEN** `sdd-router` evaluates sizing, routing, slicing or escalation inputs for a project with `.lufy/config/project.yaml`
...
- **GIVEN** `.lufy/config/project.yaml` or `workflow_limits.stop_rules` is not available

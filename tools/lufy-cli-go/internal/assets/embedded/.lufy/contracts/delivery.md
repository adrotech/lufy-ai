...
- **Review workload gate**: for T1/T2 review slices with a known Git base, consume `lufy-ai context review --base <ref> --json` when available. `split`/`escalate` must remain visible; `proceed` is secondary evidence and cannot replace validation, review score, explicit delivery authorization, remote checks, sync, or closure gates. Missing/stale graph data must preserve direct Git observations and report traceability as `unknown` with recovery.
...
- **PR ignored/internal path gate** for `validator`/`delivery`: before push, PR creation, or PR-ready reporting, run `lufy-ai pr guard --base origin/develop` when the CLI is available; for pending local changes before commit, use `lufy-ai pr guard --base origin/develop --include-worktree`. This detects files in the PR diff that match `.gitignore` via `git check-ignore -v --no-index --stdin` and known internal metadata paths such as `openspec/`, `.lufy/`, `.lufy-ai/` and `pr_review/`. OpenSpec canonical specs under `openspec/specs/`, archive records under `openspec/changes/archive/`, and deleted active change files during archive are versionable for sync/archive PRs; active `openspec/changes/<id>/` files added or modified remain internal. The neutral contract root `.lufy/contracts/` and the managed source root `.lufy/sdd/` are versionable because they feed the installed harness. Declared user-owned Lufy SDD roots under `.lufy/workflows/sdd/` (`changes/`, `specs/`, `decisions/`, `verification/`, `archive/`) are exempt only from the generic internal-prefix match; an explicit `.gitignore` match still blocks every versionable root. If the CLI is unavailable, use the fallback `git diff --name-only origin/develop...HEAD -- | git check-ignore -v --no-index --stdin` plus the same manual classification. Block delivery or require explicit user override recorded in the PR/body when real matches appear, because `.gitignore` does not prevent already tracked files or cherry-picked commits from entering a PR.
...
- Numeric review budgets use only `workflow_limits.review` (`max_files_per_slice`, `max_churn_lines_per_slice`, `max_concurrent_slices`, `min_evidence_items`). Missing config or fields are `not_available`; top-level legacy aliases are non-canonical.

...
- `--demo`: preparar una demo T3 dummy adaptada al stack declarado en `.lufy/config/project.yaml`.
...
3. Lee `.lufy/config/project.yaml` como fuente canónica de stacks y toolchain.
...
- Si falta `.lufy/config/project.yaml`, recomendar `lufy-ai init [--target <path>]` antes de continuar.
- Si el stack es unsupported, `supported: false` o contiene comandos `TODO`, explicar el flujo de forma estática y pedir completar `.lufy/config/project.yaml`.

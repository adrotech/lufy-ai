...
- Resolve the effective source layer in this order when possible: `openspec` CLI in `PATH`, `.lufy-ai/openspec-cache/<version>/manifest.json`, then embedded/local baseline metadata.
...
3. If PATH is unavailable/incompatible, inspect `.lufy-ai/openspec-cache/<version>/manifest.json` entries and reject corrupt manifests, unsafe paths or symlink escapes.

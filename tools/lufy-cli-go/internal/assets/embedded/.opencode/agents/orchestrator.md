...
- At a T1/T2 review-slice boundary with a known Git base, route execution of `lufy-ai context review --base <ref> --json` to a shell-capable role and carry its bounded action, violations, freshness and recovery as secondary evidence. Never let `proceed` advance validation, delivery or closure by itself; make `split`/`escalate` visible before continuing.
...
- Confirm an assignment only while its recommendation remains the latest projection event and the authoritative CAS still has global capacity and actor budget. A current ledger version never revives a displaced recommendation.
- A safe yield releases adaptive lease/budget only after a durable `recorded` or equivalent `duplicate_noop` checkpoint receipt. Conflict, stale lease, owner mismatch, unavailable storage, or ordinary yield after expiry leaves the assignment active. Recover an expired lease only with exact fencing and a durable `lease_expiring` checkpoint to `waiting`; time alone never releases it.
...
- Preserve a supplied `surface-execution-plan/v1` across handoffs. When it is missing and surface ambiguity or a contract boundary affects routing, ask `explorer` to generate the read-only plan; orchestrator must not execute shell itself or silently guess between frontend, backend and composed fullstack scope.
...
- Resolve skills local-first. Prefer `.lufy/skill-registry.json` when it is ready, pass each selected skill's exact `path` in downstream handoffs, and require the receiver to read the original `SKILL.md` instead of relying on a partial summary. If the registry is missing or stale, report `lufy-ai skills ensure --target <repo>` as recovery and fall back to the active adapter's local skill root. If local skills are insufficient, only suggest external bootstrap as an optional dry run such as `npx autoskills --dry-run`; never execute mutating bootstrap without explicit authorization.

...
- When `methodology_id: lufy-sdd`, include the native change summary, mode, source artifacts and automatic overview path in PR context. Full requires current sync state; Lite records sync as `not_applicable`.
...
- Before pushing, creating a PR, or reporting PR-ready state, run the ignored/internal path gate for the target base. Prefer `lufy-ai pr guard --base origin/develop` (or the explicit target base). If the CLI is unavailable, use `git diff --name-only origin/develop...HEAD -- | git check-ignore -v --no-index --stdin` and manually inspect internal metadata prefixes `openspec/`, `.lufy/`, `.lufy-ai/`, `pr_review/`. Do not flag OpenSpec canonical specs, OpenSpec archive records, active OpenSpec change deletions during archive, the versionable managed source root `.lufy/sdd/`, or declared user-owned Lufy SDD roots (`changes/`, `specs/`, `decisions/`, `verification/`, `archive/`) solely because they live under an internal prefix; an explicit `.gitignore` match still blocks.
...
- Preserve any carried `context review` action/violations in PR evidence. `split` or `escalate` blocks delivery readiness until resolved or explicitly accepted by the proper human; `proceed` remains advisory and never grants Git/GH authorization or overrides checks, sync, traceability, or closure gates.
...
- Do not archive Lufy SDD changes with incomplete tasks, missing evidence, stale Full sync, failed checks or pending delivery. Lite sync is `not_applicable`, not a blocker.

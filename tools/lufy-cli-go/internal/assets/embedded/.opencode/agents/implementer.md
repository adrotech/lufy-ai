...
- When the handoff contains `surface-execution-plan/v1`, use its active surfaces and typed validation rules instead of re-resolving frontend/backend scope. Do not execute suggested commands merely because they appear in the plan; project permissions and grouped validation policy still apply.

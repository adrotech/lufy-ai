...
- Antes de la primera resolución de skills, inspecciona `.lufy/skill-registry.json`; si falta o está stale, ejecuta `lufy-ai skills ensure --target <repo> --tool codex`. `install`, `setup` y `sync` también realizan este ensure best-effort. No asumas que `.codex/hooks.json` implementa un hook de inicio mientras Codex no exponga un contrato nativo verificado.

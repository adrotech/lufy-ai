...
- **WHEN** the user runs `lufy-ai init --target <dir>` and `<dir>/.lufy/config/project.yaml` does not exist
- **THEN** the CLI creates `.lufy/config/project.yaml` with `schema_version`, `detected_at`, `project_profile`, `stacks`, `ci`, `tdd` and `workflow_limits`
...
- **WHEN** the user runs `lufy-ai init --target <dir>` and `<dir>/.lufy/config/project.yaml` does not exist
- **THEN** the generated `.lufy/config/project.yaml` contains top-level `workflow_limits` with `sizing`, `routing`, `proposal_slicing_strategy`, `delivery_batch_strategy`, `stop_rules` and `preflight`

#### Scenario: Proposal slicing and delivery batching are distinct
- **WHEN** `.lufy/config/project.yaml` is generated or rescanned
...
Top-level `loc_budget` and `delivery_strategy` in `.lufy/config/project.yaml` SHALL NOT be valid workflow-limit sources.

#### Scenario: Legacy top-level fields detected during rescan
- **GIVEN** `.lufy/config/project.yaml` contains top-level `loc_budget` or `delivery_strategy`
...
- **WHEN** the user runs `lufy-ai init --target <dir>` and a new `.lufy/config/project.yaml` is written
...
- **THEN** `.lufy/config/project.yaml` includes a stack with `id: go`, `supported: true`, package manager `go modules`, Go test, formatter, static analysis and default coverage settings
...
- **THEN** `.lufy/config/project.yaml` includes a stack with `id: typescript`, `supported: true`, frameworks including `react` and `next`, and detected JavaScript package manager, test runner, linter and formatter when present
...
- **THEN** `.lufy/config/project.yaml` includes a stack with `id: javascript`, `supported: true` and JavaScript tooling inferred from `package.json`

#### Scenario: Python stack detected
- **WHEN** the target contains `pyproject.toml`, `requirements.txt` or `setup.py`
- **THEN** `.lufy/config/project.yaml` includes a stack with `id: python`, `supported: true` and Python test, lint, format and static analysis commands inferred from available files

#### Scenario: Java or Kotlin stack detected
- **WHEN** the target contains `pom.xml`, `build.gradle` or `build.gradle.kts`
- **THEN** `.lufy/config/project.yaml` includes a supported JVM stack with Maven or Gradle commands inferred from the build file
...
- **THEN** `.lufy/config/project.yaml` includes a stack with `id: rust`, `supported: false`, placeholder commands and notes that official support is pending
...
`lufy-ai init` SHALL include detected test runner, formatter, linter, static analysis, observability and CI metadata in `.lufy/config/project.yaml`.
...
- **THEN** `.lufy/config/project.yaml` includes `ci.detected: true`, provider metadata and known workflow paths
...
The installed harness SHALL provide a local format-dispatch hook that uses `.lufy/config/project.yaml` formatter and linter autofix metadata for changed files without assuming a fixed language toolchain.

#### Scenario: Hook formats supported stack files
- **WHEN** `.opencode/hooks/format-dispatch.sh` receives a changed `.go`, `.ts`, `.tsx` or `.py` file whose extension matches a supported stack formatter in `.lufy/config/project.yaml`
...
- **WHEN** the hook receives a file with an unmatched extension, a file from an unsupported stack, a missing `.lufy/config/project.yaml`, or an empty/TODO formatter command
...
`lufy-ai init --rescan` SHALL merge newly detected stack and surface evidence into an existing `.lufy/config/project.yaml` without discarding user-managed preferences.

#### Scenario: Coverage override preserved
- **WHEN** `.lufy/config/project.yaml` contains `coverage_threshold: 70` for stack `go` and the user runs `lufy-ai init --rescan`
- **THEN** the resulting config preserves `coverage_threshold: 70` for stack `go`

#### Scenario: Workflow limits override preserved
- **WHEN** `.lufy/config/project.yaml` contains user-managed overrides under `workflow_limits` and the user runs `lufy-ai init --rescan`
...
- **WHEN** `.lufy/config/project.yaml` contains user-managed `project_profile.surfaces` entries and the user runs `lufy-ai init --rescan`
...
`lufy-ai init --rescan` SHALL compare an existing `.lufy/config/project.yaml` with the target repository's current stack, tooling and CI evidence before deciding whether to write changes.

#### Scenario: No drift keeps project config unchanged
- **GIVEN** `.lufy/config/project.yaml` matches the current detectable stack, tooling and CI evidence
- **WHEN** the user runs `lufy-ai init --target <dir> --rescan`
- **THEN** the CLI reports no drift and MUST NOT rewrite `.lufy/config/project.yaml`

#### Scenario: New stack evidence is reported and merged
- **GIVEN** `.lufy/config/project.yaml` contains stack `go`
- **WHEN** the target later contains TypeScript/Next marker files and the user runs `lufy-ai init --target <dir> --rescan`
- **THEN** the CLI reports a new stack drift item for `typescript` and writes a merged `.lufy/config/project.yaml` that preserves the existing Go stack and user-managed fields

#### Scenario: Tooling drift is reported per stack
- **GIVEN** `.lufy/config/project.yaml` contains a JavaScript stack without a detected test runner
...
- **GIVEN** `.lufy/config/project.yaml` contains stack `python` with user-managed preferences
...
- **THEN** the CLI MUST NOT delete source files, configuration files, `.lufy/config/project.yaml` entries or unknown user files as part of stale handling
...
- **GIVEN** `.lufy/config/project.yaml` contains an unknown top-level field and an unknown field under stack `go`
- **WHEN** the user runs `lufy-ai init --target <dir> --rescan`
- **THEN** the resulting `.lufy/config/project.yaml` preserves both unknown fields unless the command fails before writing

#### Scenario: User override wins over regenerated default
- **GIVEN** `.lufy/config/project.yaml` contains a user override such as `coverage_threshold: 70` for stack `go`
...
- **THEN** the resulting `.lufy/config/project.yaml` preserves `coverage_threshold: 70` and reports that the user-managed override was preserved when reporting is verbose enough to include preserved decisions

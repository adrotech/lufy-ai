...
func TestBuildCatalogExpandsManagedAssetsAndExcludesOpenSpecChanges(t *testing.T) {
	...
	want := map[string]bool{
		...
		filepath.Join(".agents", "skills", "git-delivery", "agents", "openai.yaml"):            false,
		filepath.Join(".agents", "skills", "pr-reviewer", "references", "review-framework.md"): false,
		filepath.Join(".agents", "skills", "pr-reviewer", "assets", "report.html"):             false,
		filepath.Join(".agents", "skills", "pr-reviewer", "agents", "openai.yaml"):             false,
...
func TestEmbeddedCatalogMatchesRepositoryAssets(t *testing.T) {
	...
	if !reflect.DeepEqual(rootAssets, embeddedAssets) {
		limit := len(rootAssets)
		if len(embeddedAssets) < limit {
			limit = len(embeddedAssets)
		}
		for i := 0; i < limit; i++ {
			if rootAssets[i] != embeddedAssets[i] {
				t.Fatalf("root and embedded catalogs drifted at index %d\nroot=%#v\nembedded=%#v", i, rootAssets[i], embeddedAssets[i])
			}
		}
		t.Fatalf("root and embedded catalogs differ in length: root=%d embedded=%d", len(rootAssets), len(embeddedAssets))
	}
}
...
func TestAgentAssetsCoverT2ApprovalConversationScenarios(t *testing.T) {
	...
	scenarios := []struct {
	...
	}{
		{
			...
			text: orchestrator,
			want: []string{
				"Treat non-trivial `T1` or `T2` routes with `fast_path_allowed: false` as an explicit user decision gate",
				"Before invoking `implementer` or continuing into implementation, present a visible human plan",
		...
		},
		{
			...
			text: spec,
			want: []string{
				"T1/T2 non-trivial work requires post-plan approval",
...
func minimalSource(t *testing.T) string {
	...
	files := map[string]string{
		...
		filepath.Join(".agents", "skills", "git-delivery", "agents", "openai.yaml"):       "interface:\n  display_name: Git Delivery\n",
		filepath.Join(".agents", "skills", "lufy-close", "SKILL.md"):                      "close skill\n",
		filepath.Join(".agents", "skills", "pr-reviewer", "SKILL.md"):                     "pr reviewer skill\n",
		filepath.Join(".agents", "skills", "pr-reviewer", "agents", "openai.yaml"):        "interface:\n  display_name: PR Reviewer\n",

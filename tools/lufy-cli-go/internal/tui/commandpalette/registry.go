...
func Registry() []CommandSpec {
	...
	inputFile := ParamSpec{Name: "file", Flag: "--file", Kind: ParamText, Required: true, Description: "Archivo YAML/JSON de entrada"}
	idempotencyKey := ParamSpec{Name: "idempotency-key", Flag: "--idempotency-key", Kind: ParamText, Required: true, Description: "Clave estable de retry"}
	...
	return []CommandSpec{
		...
		{ID: "plan", Title: "Surface Plan", Description: "Resuelve superficies y validaciones", Args: []string{"plan"}, Params: []ParamSpec{commonTarget, {Name: "surface", Flag: "--surface", Kind: ParamText, Default: "auto", Description: "ID o tipo de superficie"}, {Name: "base", Flag: "--base", Kind: ParamText, Default: "HEAD", Description: "Referencia Git base"}, {Name: "files", Flag: "--files", Kind: ParamText, Description: "Paths separados por coma"}, {Name: "capabilities", Flag: "--capabilities", Kind: ParamText, Description: "Capacidades separadas por coma"}, jsonOut}},
		...
		{ID: "skills-refresh", Title: "Skills Refresh", Description: "Actualiza el registry portable de skills", Args: []string{"skills", "refresh"}, Params: []ParamSpec{commonTarget, skillTool, jsonOut}},
		{ID: "skills-status", Title: "Skills Status", Description: "Diagnostica el registry portable de skills", Args: []string{"skills", "status"}, Params: []ParamSpec{commonTarget, skillTool, jsonOut}},
		...
		{ID: "sdd-new", Title: "Lufy SDD New", Description: "Crea un change Full/Lite con overview", Args: []string{"sdd", "new"}, Params: []ParamSpec{commonTarget, {Name: "change", Flag: "--change", Kind: ParamText, Required: true, Description: "Change ID"}, {Name: "mode", Flag: "--mode", Kind: ParamChoice, Default: "full", Choices: []string{"full", "lite"}, Description: "Mode del change"}, {Name: "capability", Flag: "--capability", Kind: ParamText, Description: "Capability ID (Full)"}, jsonOut}},
		{ID: "sdd-status", Title: "Lufy SDD Status", Description: "Resume un change Lufy SDD", Args: []string{"sdd", "status"}, Params: []ParamSpec{commonTarget, {Name: "change", Flag: "--change", Kind: ParamText, Required: true, Description: "Change ID"}, jsonOut}},
		...
		{ID: "adaptive-recommend", Title: "Adaptive Recommend", Description: "Evalúa demanda/capacidades sin mutar por default", Args: []string{"adaptive", "recommend"}, Params: []ParamSpec{commonTarget, inputFile, {Name: "run", Flag: "--run", Kind: ParamText, Description: "Run ID opcional"}, {Name: "record", Flag: "--record", Kind: ParamBool, Description: "Registrar en advisory"}, {Name: "idempotency-key", Flag: "--idempotency-key", Kind: ParamText, Description: "Clave requerida con recording"}, jsonOut}},
		{ID: "adaptive-assign", Title: "Adaptive Assign", Description: "Confirma recommendation vigente con lease fenced", Args: []string{"adaptive", "assign"}, Params: []ParamSpec{commonTarget, runID, inputFile, {Name: "record", Flag: "--record", Kind: ParamBool, Default: "true", Description: "Confirmar efecto durable"}, idempotencyKey, jsonOut}},
		{ID: "adaptive-yield", Title: "Adaptive Yield", Description: "Registra checkpoint y libera recursos adaptativos", Args: []string{"adaptive", "yield"}, Params: []ParamSpec{commonTarget, runID, inputFile, {Name: "record", Flag: "--record", Kind: ParamBool, Default: "true", Description: "Confirmar efecto durable"}, idempotencyKey, jsonOut}},
		{ID: "adaptive-status", Title: "Adaptive Status", Description: "Reconstruye assignments, budget y waiting pool", Args: []string{"adaptive", "status"}, Params: []ParamSpec{commonTarget, runID, jsonOut}},
